...
class MainViz:
    def __init__(
            self, joint_info_pred, joint_info_true, joint_info_alt, camera_type, show_image,
        ...
        if joint_info_true[0] is not None:
            self.skeletons_true = poseviz.components.skeletons_viz.SkeletonsViz(
                joint_info_true, colors.red, colors.red, colors.red, colors.red, 0.06,
                high_quality, opacity=0.95)
        else:
        ...
        if joint_info_alt[0] is not None:
            self.skeletons_alt = poseviz.components.skeletons_viz.SkeletonsViz(
                joint_info_alt, colors.orange, colors.orange, colors.orange, colors.orange, 0.06,
                high_quality, opacity=0.95)
        else:
        ...
        if body_model_faces is not None:
            ...
            self.meshes_pred2 = poseviz.components.smpl_viz.SMPLViz(
                color=colors.orange, faces=body_model_faces, add_wireframe=True,
                colormap='Oranges_r')
        else:
    ...
    def update(
            self, camera_display, image, poses_pred=None, poses_true=None, poses_alt=None,
            vertices_pred=None, vertices_true=None, vertices_alt=None,
            highlight=False):
    ...
    def remove(self):
        if self.is_initialized:
            for elem in [
                self.skeletons_pred, self.skeletons_true, self.skeletons_alt, self.meshes_pred,
                self.meshes_gt, self.meshes_pred2]:
