        ...
        if len(c) == self.n_rendered_verts:
            ...
            if len(mayavi_poses) == 0:
                ...
                triangles = np.concatenate(
                    [np.asarray(self.faces) + i * num_vertices
                     for i in range(len(mayavi_poses))], axis=0)
            ...
            if u is not None:
                ...
                self.mesh.mlab_source.reset(
                    x=c[:, 0], y=c[:, 1], z=c[:, 2], triangles=triangles)
    ...
    def initial_update(self, poses):
        ...
        if u is not None:
            self.mesh = mlab.triangular_mesh(
                *c.T, triangles, scalars=u, colormap='cool', vmin=0, vmax=0.1,
            ...
            self.mesh = mlab.triangular_mesh(
                *c.T, triangles, color=self.color,
                opacity=self.opacity,
                representation='surface',
                reset_zoom=False)
        ...
        if self.add_wireframe:
            attrs.update(edge_visibility=True, edge_color=(0., 0., 0.), line_width=0.1)
